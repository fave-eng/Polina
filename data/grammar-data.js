/** Grammar path for Polina. */
window.GRAMMAR_DATA = [
  {
    "id": "a1-1-basics",
    "order": 1,
    "title": "Основы английской грамматики",
    "level": "B1",
    "status": "available",
    "page": "grammar-topic.html?id=a1-1-basics",
    "explanation": "Часть речи определяется не только формой слова, но и его ролью и местом в предложении. Одно и то же слово иногда может быть существительным, прилагательным или наречием.",
    "formula": "word form + position + context = part of speech",
    "table": {
      "headers": [
        "Word",
        "Meaning / use",
        "Example"
      ],
      "rows": [
        [
          "hard",
          "усердно; тяжело",
          "She works hard."
        ],
        [
          "hardly",
          "едва; почти не",
          "I could hardly hear."
        ],
        [
          "late",
          "поздно",
          "He arrived late."
        ],
        [
          "lately",
          "в последнее время",
          "Have you seen her lately?"
        ],
        [
          "near",
          "близко",
          "Come near."
        ],
        [
          "nearly",
          "почти",
          "It is nearly finished."
        ]
      ]
    },
    "examples": [
      "coffee shop, customs officer, government policy — the first noun describes the second noun.",
      "The soup tastes amazing. / He is tasting the soup.",
      "a fast train / The train went fast.",
      "She works hard. / She hardly works."
    ],
    "commonMistakes": [
      "The first noun in a noun + noun combination is usually singular.",
      "Not every word ending in -ly is an adverb: friendly, lovely, lonely and costly are adjectives.",
      "Hard and hardly have different meanings."
    ],
    "quiz": [
      {
        "prompt": "What part of speech is customs in “a customs official”?",
        "options": [
          "adjective",
          "noun",
          "adverb"
        ],
        "answer": 1,
        "explanation": "It is a noun used before another noun."
      },
      {
        "prompt": "What part of speech is fast in “They drive fast”?",
        "options": [
          "adjective",
          "adverb",
          "noun"
        ],
        "answer": 1
      },
      {
        "prompt": "The server was down, so I could ___ load the page.",
        "options": [
          "hard",
          "hardly",
          "highly"
        ],
        "answer": 1
      },
      {
        "prompt": "Which word is not normally an adverb?",
        "options": [
          "shortly",
          "hardly",
          "costly",
          "lately"
        ],
        "answer": 2
      },
      {
        "prompt": "Which sentence uses hardly correctly?",
        "options": [
          "She works hardly every day.",
          "I could hardly see anything.",
          "The exercise was hardly."
        ],
        "answer": 1
      }
    ]
  },
  {
    "id": "a1-2-articles",
    "order": 2,
    "title": "Артикли: a / an, the и нулевой артикль",
    "level": "B1",
    "status": "available",
    "page": "grammar-topic.html?id=a1-2-articles",
    "explanation": "Выбор артикля зависит от смысла: говорим ли мы об одном неопределённом предмете, о конкретном известном предмете или о понятии в целом.",
    "formula": "a/an = one non-specific item · the = specific/known · Ø = general meaning",
    "table": {
      "headers": [
        "Article",
        "Use",
        "Example"
      ],
      "rows": [
        [
          "a / an",
          "one non-specific singular countable noun",
          "I saw a doctor."
        ],
        [
          "the",
          "specific, known or unique",
          "The doctor was very helpful."
        ],
        [
          "Ø",
          "plural or uncountable nouns in general",
          "Doctors work long hours."
        ]
      ]
    },
    "examples": [
      "go to hospital as a patient, but go to the hospital to visit or work there.",
      "I ordered a coffee. / Coffee contains caffeine.",
      "The sun, the internet, the station near my house."
    ],
    "commonMistakes": [
      "Do not use a/an with plural or uncountable nouns.",
      "Use the only when the listener can identify the noun.",
      "Institutions such as school, hospital and prison may have zero article when used for their main purpose."
    ],
    "quiz": [
      {
        "prompt": "My father went to ___ hospital to fix the roof.",
        "options": [
          "hospital",
          "a hospital",
          "the hospital"
        ],
        "answer": 2
      },
      {
        "prompt": "I saw ___ woman. ___ woman was carrying a red bag.",
        "options": [
          "a / The",
          "the / A",
          "an / The"
        ],
        "answer": 0
      },
      {
        "prompt": "___ information is useful.",
        "options": [
          "A",
          "The",
          "No article"
        ],
        "answer": 2
      },
      {
        "prompt": "We met at ___ station near my house.",
        "options": [
          "a",
          "the",
          "no article"
        ],
        "answer": 1
      },
      {
        "prompt": "She wants to buy ___ new laptop.",
        "options": [
          "a",
          "an",
          "the"
        ],
        "answer": 0
      }
    ]
  },
  {
    "id": "a1-3-pronouns",
    "order": 3,
    "title": "Местоимения",
    "level": "B1",
    "status": "available",
    "page": "grammar-topic.html?id=a1-3-pronouns",
    "explanation": "Личные, объектные, притяжательные и возвратные местоимения занимают разные позиции. Притяжательное прилагательное стоит перед существительным, а абсолютное притяжательное местоимение используется самостоятельно.",
    "formula": "my book → the book is mine · their house → the house is theirs",
    "table": {
      "headers": [
        "Subject",
        "Object",
        "Possessive adjective",
        "Possessive pronoun",
        "Reflexive"
      ],
      "rows": [
        [
          "I",
          "me",
          "my",
          "mine",
          "myself"
        ],
        [
          "you",
          "you",
          "your",
          "yours",
          "yourself / yourselves"
        ],
        [
          "he",
          "him",
          "his",
          "his",
          "himself"
        ],
        [
          "she",
          "her",
          "her",
          "hers",
          "herself"
        ],
        [
          "we",
          "us",
          "our",
          "ours",
          "ourselves"
        ],
        [
          "they",
          "them",
          "their",
          "theirs",
          "themselves"
        ]
      ]
    },
    "examples": [
      "This is my book. The book is mine.",
      "She is a friend of mine.",
      "Someone left their phone. They may come back for it."
    ],
    "commonMistakes": [
      "Do not add an apostrophe to possessive pronouns: yours, hers, ours, theirs.",
      "Say a friend of mine, not a friend of my.",
      "Its is possessive; it’s means it is or it has."
    ],
    "quiz": [
      {
        "prompt": "Which sentence is correct?",
        "options": [
          "This bag is their and that one is her.",
          "This bag is theirs and that one is hers.",
          "This bag is theirs and that one is her’s."
        ],
        "answer": 1
      },
      {
        "prompt": "They are friends ___ .",
        "options": [
          "of their",
          "of theirs",
          "of them"
        ],
        "answer": 1
      },
      {
        "prompt": "Someone left ___ umbrella. Perhaps ___ will return.",
        "options": [
          "their / they",
          "his / he only",
          "theirs / them"
        ],
        "answer": 0
      },
      {
        "prompt": "That seat is ___ .",
        "options": [
          "my",
          "mine",
          "me"
        ],
        "answer": 1
      },
      {
        "prompt": "I need a pen. Have you got ___?",
        "options": [
          "one",
          "itself",
          "mine"
        ],
        "answer": 0
      }
    ]
  },
  {
    "id": "b1-ed-ing-adjectives",
    "order": 1,
    "category": "lesson",
    "lessonNumber": 10,
    "lessonTitle": "How Food Affects Mood",
    "title": "Прилагательные с -ed и -ing",
    "level": "B1",
    "status": "available",
    "page": "grammar-topic.html?id=b1-ed-ing-adjectives",
    "explanation": "Прилагательные с окончаниями -ed и -ing часто образуются от одного глагола, но передают разный смысл. Форма с -ed описывает чувство или состояние того, кто испытывает воздействие. Форма с -ing описывает человека, предмет, событие или ситуацию, которые это чувство вызывают.",
    "formula": "Кто испытывает чувство? → -ed. Кто или что вызывает чувство? → -ing.",
    "ruleCards": [
      {
        "title": "-ed: чувство или состояние",
        "text": "Используем, когда говорим о том, как человек или другое живое существо себя чувствует.",
        "example": "I’m bored.",
        "translation": "Мне скучно."
      },
      {
        "title": "-ing: причина чувства",
        "text": "Используем, когда описываем того или то, что создаёт впечатление и вызывает чувство.",
        "example": "This book is boring.",
        "translation": "Эта книга скучная."
      }
    ],
    "image": {
      "src": "assets/grammar-ed-ing.png",
      "alt": "Схема: окончание -ed описывает чувства человека, а -ing — предмет или явление, вызывающее чувство",
      "caption": "-ed показывает состояние; -ing показывает причину этого состояния."
    },
    "sections": [
      {
        "title": "Как выбрать окончание",
        "items": [
          "Найдите слово, которое нужно описать.",
          "Спросите: оно испытывает чувство или вызывает его?",
          "Испытывает чувство или находится под воздействием → -ed. Вызывает чувство или создаёт впечатление → -ing."
        ]
      },
      {
        "title": "Не только люди и предметы",
        "paragraphs": [
          "Правило нельзя сводить к схеме «человек — -ed, предмет — -ing». Важно не то, кем или чем является существительное, а какую роль оно играет в ситуации."
        ],
        "items": [
          "an annoying person — раздражающий человек: человек вызывает раздражение.",
          "an interested audience — заинтересованная аудитория: люди испытывают интерес.",
          "The journey was exhausting. — Поездка была изнурительной.",
          "We were exhausted. — Мы были измотаны."
        ]
      },
      {
        "title": "Типичные конструкции",
        "items": [
          "be / feel / seem / look + -ed: She feels disappointed.",
          "be + -ing: The result is disappointing.",
          "-ing + noun: an exciting story, a confusing explanation.",
          "-ed + preposition: interested in, excited about, surprised by, disappointed with, worried about."
        ]
      }
    ],
    "tableTitle": "Частые пары и разница в значении",
    "table": {
      "headers": [
        "-ed: состояние",
        "-ing: причина"
      ],
      "rows": [
        [
          "bored — испытывающий скуку",
          "boring — скучный"
        ],
        [
          "interested — заинтересованный",
          "interesting — интересный"
        ],
        [
          "excited — взволнованный",
          "exciting — захватывающий"
        ],
        [
          "confused — растерянный",
          "confusing — запутанный"
        ],
        [
          "surprised — удивлённый",
          "surprising — удивительный, неожиданный"
        ],
        [
          "annoyed — раздражённый",
          "annoying — раздражающий"
        ],
        [
          "disappointed — разочарованный",
          "disappointing — разочаровывающий"
        ],
        [
          "exhausted — измотанный",
          "exhausting — изнурительный"
        ],
        [
          "embarrassed — смущённый",
          "embarrassing — неловкий, вызывающий смущение"
        ],
        [
          "shocked — потрясённый",
          "shocking — шокирующий"
        ]
      ]
    },
    "examplesTitle": "Сравните предложения",
    "examples": [
      "I was surprised when I saw the scenery. — Я удивилась, когда увидела пейзаж.",
      "The scenery was surprising. — Пейзаж оказался удивительным.",
      "I’m interested in the characters. — Мне интересны персонажи.",
      "The characters are interesting. — Персонажи интересные.",
      "We felt exhausted after the journey. — После поездки мы чувствовали себя измотанными.",
      "The journey was exhausting. — Поездка была изнурительной."
    ],
    "commonMistakesTitle": "Что важно не перепутать",
    "commonMistakes": [
      "I am boring означает «я скучный, я вызываю скуку». Чтобы сказать «мне скучно», нужно I am bored.",
      "I am interesting означает «я интересный». Чтобы сказать «мне интересно», обычно нужно I am interested.",
      "Окончание выбирают по смыслу предложения, а не просто по тому, говорится ли о человеке или о предмете."
    ],
    "quiz": [
      {
        "prompt": "I felt very ___ after the long meeting.",
        "options": [
          "tiring",
          "tired"
        ],
        "answer": 1
      },
      {
        "prompt": "The instructions were ___, so nobody knew what to do.",
        "options": [
          "confused",
          "confusing"
        ],
        "answer": 1
      },
      {
        "prompt": "She is ___ in modern art.",
        "options": [
          "interested",
          "interesting"
        ],
        "answer": 0
      },
      {
        "prompt": "It was an ___ match from beginning to end.",
        "options": [
          "excited",
          "exciting"
        ],
        "answer": 1
      },
      {
        "prompt": "We were ___ by the sudden change.",
        "options": [
          "surprised",
          "surprising"
        ],
        "answer": 0
      },
      {
        "prompt": "The noise is really ___.",
        "options": [
          "annoyed",
          "annoying"
        ],
        "answer": 1
      },
      {
        "prompt": "He looked ___ when he heard the result.",
        "options": [
          "disappointed",
          "disappointing"
        ],
        "answer": 0
      },
      {
        "prompt": "The trip was ___, but the views were beautiful.",
        "options": [
          "exhausted",
          "exhausting"
        ],
        "answer": 1
      }
    ]
  },
  {
    "id": "a1-4-locked",
    "order": 4,
    "title": "Настоящее время",
    "level": "B1",
    "status": "locked",
    "page": "grammar-topic.html?id=a1-4-locked",
    "explanation": "Материал будет опубликован позже.",
    "quiz": []
  },
  {
    "id": "a1-5-locked",
    "order": 5,
    "title": "Прошедшее время",
    "level": "B1",
    "status": "locked",
    "page": "grammar-topic.html?id=a1-5-locked",
    "explanation": "Материал будет опубликован позже.",
    "quiz": []
  },
  {
    "id": "a1-6-locked",
    "order": 6,
    "title": "Будущее время с will",
    "level": "B1",
    "status": "locked",
    "page": "grammar-topic.html?id=a1-6-locked",
    "explanation": "Материал будет опубликован позже.",
    "quiz": []
  },
  {
    "id": "a1-7-locked",
    "order": 7,
    "title": "Предлоги времени и места",
    "level": "B1",
    "status": "locked",
    "page": "grammar-topic.html?id=a1-7-locked",
    "explanation": "Материал будет опубликован позже.",
    "quiz": []
  },
  {
    "id": "a1-8-locked",
    "order": 8,
    "title": "Вопросительные структуры",
    "level": "B1",
    "status": "locked",
    "page": "grammar-topic.html?id=a1-8-locked",
    "explanation": "Материал будет опубликован позже.",
    "quiz": []
  },
  {
    "id": "a1-9-locked",
    "order": 9,
    "title": "Краткие ответы",
    "level": "B1",
    "status": "locked",
    "page": "grammar-topic.html?id=a1-9-locked",
    "explanation": "Материал будет опубликован позже.",
    "quiz": []
  },
  {
    "id": "a1-10-locked",
    "order": 10,
    "title": "There is / There are",
    "level": "B1",
    "status": "locked",
    "page": "grammar-topic.html?id=a1-10-locked",
    "explanation": "Материал будет опубликован позже.",
    "quiz": []
  },
  {
    "id": "a1-11-locked",
    "order": 11,
    "title": "Союзы and, but, because",
    "level": "B1",
    "status": "locked",
    "page": "grammar-topic.html?id=a1-11-locked",
    "explanation": "Материал будет опубликован позже.",
    "quiz": []
  },
  {
    "id": "a1-12-locked",
    "order": 12,
    "title": "Порядок слов",
    "level": "B1",
    "status": "locked",
    "page": "grammar-topic.html?id=a1-12-locked",
    "explanation": "Материал будет опубликован позже.",
    "quiz": []
  },
  {
    "id": "a1-13-locked",
    "order": 13,
    "title": "Неправильные глаголы",
    "level": "B1",
    "status": "locked",
    "page": "grammar-topic.html?id=a1-13-locked",
    "explanation": "Материал будет опубликован позже.",
    "quiz": []
  },
  {
    "id": "a1-14-locked",
    "order": 14,
    "title": "Множественное число",
    "level": "B1",
    "status": "locked",
    "page": "grammar-topic.html?id=a1-14-locked",
    "explanation": "Материал будет опубликован позже.",
    "quiz": []
  },
  {
    "id": "a1-15-locked",
    "order": 15,
    "title": "To be going to",
    "level": "B1",
    "status": "locked",
    "page": "grammar-topic.html?id=a1-15-locked",
    "explanation": "Материал будет опубликован позже.",
    "quiz": []
  }
];
